...
class RAGPipeline:
    ...
    def discover_topics(self, sample_size: int = 50) -> List[str]:
        ...
        # Improved topic discovery prompt
        topic_prompt = PromptTemplate(
            input_variables=["context"],
            template="""Analyze this context and identify 5-7 main topics.

Context:
{context}

Return ONLY a valid JSON array of topic strings with NO additional text.
...
JSON array:"""
        ...
        result = topic_chain.invoke({"context": context[:8000]})
        ...
        try:
            ...
            if matches and len(matches) >= 3:
                ...
                return matches[:7]  # Limit to 7 topics
    ...
    def extract_topic_updates(self, topic: str) -> List[str]:
        ...
        for i in range(0, len(retrieved_docs), batch_size):
            ...
            # Improved extraction prompt with clearer JSON instructions
            extraction_prompt = PromptTemplate(
                input_variables=["topic", "context"],
                template="""Extract all information related to: {topic}

Context:
{context}

Extract ONLY facts from the context above. Return a valid JSON array with NO additional text.
...
JSON array:"""
            ...
            try:
                # Clean the result - remove leading/trailing whitespace
                result = result.strip()
                ...
                if json_start != -1 and json_end != 0:
                    ...
                    if isinstance(batch_updates, list):
                        ...
                        batch_updates = [str(item).strip() for item in batch_updates if str(item).strip()]
                        all_updates.extend(batch_updates)
                        logger.info(f"Batch {i//batch_size}: extracted {len(batch_updates)} updates")
                    else:
                        logger.warning(f"Batch {i//batch_size}: parsed JSON is not a list")
                        
            except json.JSONDecodeError as e:
                logger.warning(f"JSON parse error in batch {i//batch_size}: {e}")
                logger.debug(f"Problematic response: {result[:200]}...")
                
                # Fallback: try to extract items manually using regex
                matches = re.findall(r'"([^"]*)"', result)
                if matches:
                    logger.info(f"Batch {i//batch_size}: extracted {len(matches)} updates using regex fallback")
                    all_updates.extend(matches)
                else:
                    logger.warning(f"Batch {i//batch_size}: could not extract any updates")
    ...
    def deduplicate_updates(self, updates: List[str]) -> List[str]:
        ...
        batch_size = 15  # Reduced for more reliable parsing
        deduplicated = []
        
        for i in range(0, len(updates), batch_size):
            batch = updates[i:i + batch_size]
            
            # Improved deduplication prompt
            dedup_prompt = PromptTemplate(
                input_variables=["updates"],
                template="""Remove duplicates from this list. Keep the most complete version of similar items.
...
Format: ["unique update 1", "unique update 2"]

JSON array:"""
            )
            
            # Create and run the deduplication chain
            ...
            try:
                ...
                if json_start != -1 and json_end != 0:
                    ...
                    if isinstance(batch_deduped, list):
                        batch_deduped = [str(item).strip() for item in batch_deduped if str(item).strip()]
                        deduplicated.extend(batch_deduped)
                        logger.info(f"Dedup batch {i//batch_size}: {len(batch)} -> {len(batch_deduped)} updates")
                    else:
                        # Fallback to original batch
                        deduplicated.extend(batch)
                        logger.warning(f"Dedup batch {i//batch_size}: using original batch (not a list)")
                        
            except json.JSONDecodeError as e:
                logger.warning(f"Deduplication parsing failed for batch {i//batch_size}: {e}")
                # Fallback: try regex extraction
                matches = re.findall(r'"([^"]*)"', result)
                if matches:
                    deduplicated.extend(matches)
                    logger.info(f"Dedup batch {i//batch_size}: extracted {len(matches)} using regex")
                else:
                    # Last resort: use original batch
                    deduplicated.extend(batch)
                    logger.warning(f"Dedup batch {i//batch_size}: using original batch (parse failed)")
                    
            except Exception as e:
                logger.error(f"Unexpected deduplication error: {e}")
                deduplicated.extend(batch)
